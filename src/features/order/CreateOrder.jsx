...
function CreateOrder() {
  ...
  const priorityPrice = prioritized ? cartPrice * 0.2 : 0;
  ...
  if(!cart.length)
    return <h2 className="px-4 py-6 text-xl font-semibold text-center">Empty card. Order something! ;)</h2>
    ...
    <div className="px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className="mb-12 flex gap-5 items-center">
          ...
          />
          <label className="font-medium" htmlFor="priority">Want to give your order priority? (+20% price)</label>
        </div>

        <div>
          <input type="hidden" name="cart" value={JSON.stringify(cart)} />
          <input type="hidden" name="position" value={position.longitude ? `${position.latitude},${position.longitude}` : "not provided"} />
          <Button disabled={isSubmitting || isLoadingAddress}>
            {isSubmitting ? "Order is being processed..." : `Order now ${formatCurrency(finalPrice)}`}
          </Button>
        </div>
      </Form>
    </div>
...
export async function action({request}) {
  ...
  // if no errors, create a new order and then redirect;
  const newOrder = await createOrder(order);

  // hack around, should not be overused;
